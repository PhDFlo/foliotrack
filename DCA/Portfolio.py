...
class PortfolioETF:
    ...
    def generate_shares_matrix(self, fixed_etf: ETF = None):
        """
        Generate a square matrix of dimension n (number of ETFs in the portfolio minus one if fixed_etf is provided).
        The diagonal values are 1 - p, where p is the portfolio share of the ETF.
        All off-diagonal values are -p.
        The ETF given by fixed_etf is not included in the matrix.

        Args:
            fixed_etf (ETF, optional): ETF instance to exclude from the matrix. Defaults to None.

        Returns:
            np.ndarray: The generated matrix as a NumPy array.
        """
        filtered_portfolio = [item for item in self.portfolio if (fixed_etf is None or item["etf"] != fixed_etf)]
    ...
    def generate_invested_vector(self, fixed_etf: ETF = None, investment_fixed_etf: float = 0.):
        """
        Generate a vector of size n (number of ETFs in the portfolio minus one if fixed_etf is provided).
        The vector is the sum of the amount invested times the unit vector (i.e., the invested amounts for each ETF).

        Args:
            fixed_etf (ETF, optional): ETF instance to exclude from the vector. Defaults to None.
            investment_fixed_etf (float, optional): Investment value for the fixed ETF. Defaults set to 0.
        ...
        # Remove the line of the fixed etf if it exist
        if (fixed_etf is not None):
            vector = np.delete(vector, index_fixed_etf)
        
        return vector

    def inverse_shares_matrix(self, fixed_etf: ETF = None):
        """
        ...
        """
        matrix = self.generate_shares_matrix(fixed_etf)
        return np.linalg.inv(matrix)

    def solve_equilibrium(self, fixed_etf: ETF = None, investment_fixed_etf: float = 0.):
        """
        Compute and update the amount to be invested for each ETF in the portfolio.
        For the fixed ETF, set the amount to be invested to 0.

        Args:
            fixed_etf (ETF, optional): ETF instance to exclude from the computation. Defaults to None.
            investment_fixed_etf (float, optional): Investment value for the fixed ETF. Defaults set to 0.
